...
export default function CIE10() {
    const { codigos, filters } = usePage<Props>().props;
    const [search, setSearch] = useState(filters.search || '');
    const [isCreateModalOpen, setIsCreateModalOpen] = useState(false);
    const [isEditModalOpen, setIsEditModalOpen] = useState(false);
    const [editingCodigo, setEditingCodigo] = useState<CodigoCIE10 | null>(null);
    const [formData, setFormData] = useState({ codigo: '', descripcion: '' });

    // Búsqueda en tiempo real
    const handleSearch = (value: string) => {
        setSearch(value);
        router.get('/admin/configuracion/cie10', { search: value }, { preserveState: true });
    };

    // Crear código
    const handleCreate = (e: React.FormEvent) => {
        e.preventDefault();
        router.post('/admin/configuracion/cie10', formData, {
            onSuccess: () => {
    ...
    const handleEdit = (e: React.FormEvent) => {
        ...
        router.put(`/admin/configuracion/cie10/${editingCodigo.id}`, formData, {
            onSuccess: () => {
    ...
    const toggleStatus = (id: number) => {
        router.patch(`/admin/configuracion/cie10/${id}/toggle`, {}, {
    ...
    const handleDelete = (id: number, codigo: string) => {
        if (confirm(`¿Está seguro de eliminar el código ${codigo}?`)) {
            router.delete(`/admin/configuracion/cie10/${id}`, {
    ...
    const openEditModal = (codigo: CodigoCIE10) => {
        setEditingCodigo(codigo);
        setFormData({ codigo: codigo.codigo, descripcion: codigo.descripcion });
            ...
            <div className="space-y-4 sm:space-y-5">
                ...
                <div className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-xl sm:text-2xl font-bold text-slate-900">Códigos CIE-10</h1>
                            <p className="text-xs sm:text-sm text-slate-600 mt-1">
                                {codigos.total} diagnósticos médicos
                            </p>
                        </div>
                    </div>
                    ...
                    <Dialog open={isCreateModalOpen} onOpenChange={setIsCreateModalOpen}>
                        ...
                        <DialogContent>
                            <form onSubmit={handleCreate}>
                                <DialogHeader>
                                    <DialogTitle>Nuevo Código CIE-10</DialogTitle>
                                    <DialogDescription>
                                        Agregue un nuevo código de diagnóstico médico
                                    </DialogDescription>
                                </DialogHeader>
                                        ...
                                        <TableBody>
                                            {codigos.data.map((codigo) => (
                                                ...
                                                >
                                                    ...
                                                    <TableCell>
                                                        <span className="text-sm text-slate-700">{codigo.descripcion}</span>
                                                    </TableCell>
                                                    <TableCell className="text-center">
                                                        <button
                                                            onClick={() => toggleStatus(codigo.id)}
                                ...
                                {codigos.last_page > 1 && (
                                    <div className="flex items-center justify-between pt-2">
                                        <div className="text-sm text-slate-600">
                                            Mostrando {codigos.from} a {codigos.to} de {codigos.total} registros
                                        </div>
                                        <div className="flex gap-2">
                                            <Button
                                                size="sm"
                                                variant="outline"
                                                disabled={codigos.current_page === 1}
                                                onClick={() => router.get(`/admin/configuracion/cie10?page=${codigos.current_page - 1}&search=${search}`)}
                                            >
                                                Anterior
                                            </Button>
                                            <Button
                                                size="sm"
                                                variant="outline"
                                                disabled={codigos.current_page === codigos.last_page}
                                                onClick={() => router.get(`/admin/configuracion/cie10?page=${codigos.current_page + 1}&search=${search}`)}
                                            >
                                                Siguiente
                                            </Button>
                ...
                <Dialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen}>
                    <DialogContent>
                        <form onSubmit={handleEdit}>
                            <DialogHeader>
                                <DialogTitle>Editar Código CIE-10</DialogTitle>
                                <DialogDescription>
                                    Modifique los datos del código de diagnóstico
                                </DialogDescription>
                            </DialogHeader>
                            <div className="space-y-4 py-4">
                                <div className="space-y-2">
