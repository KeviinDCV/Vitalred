...
export default function Servicios() {
    const { servicios, filters } = usePage<Props>().props;
    const [search, setSearch] = useState(filters.search || '');
    const [isCreateModalOpen, setIsCreateModalOpen] = useState(false);
    const [isEditModalOpen, setIsEditModalOpen] = useState(false);
    const [editingServicio, setEditingServicio] = useState<TipoServicio | null>(null);
    ...
    const handleSearch = (value: string) => {
        setSearch(value);
        router.get('/admin/configuracion/servicios', { search: value }, { preserveState: true });
    };

    // Crear tipo de servicio
    const handleCreate = (e: React.FormEvent) => {
        e.preventDefault();
        router.post('/admin/configuracion/servicios', formData, {
            onSuccess: () => {
    ...
    const handleEdit = (e: React.FormEvent) => {
        ...
        router.put(`/admin/configuracion/servicios/${editingServicio.id}`, formData, {
            onSuccess: () => {
    ...
    const toggleStatus = (id: number) => {
        router.patch(`/admin/configuracion/servicios/${id}/toggle`, {}, {
    ...
    const handleDelete = (id: number, nombre: string) => {
        if (confirm(`¿Está seguro de eliminar el tipo de servicio ${nombre}?`)) {
            router.delete(`/admin/configuracion/servicios/${id}`, {
                onSuccess: () => toast.success('Tipo de servicio eliminado'),
            });
        }
    };

    // Abrir modal de edición
    const openEditModal = (servicio: TipoServicio) => {
            ...
            <div className="space-y-4 sm:space-y-5">
                ...
                <div className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-xl sm:text-2xl font-bold text-slate-900">Tipos de Servicio</h1>
                            <p className="text-xs sm:text-sm text-slate-600 mt-1">
                                {servicios.total} servicios hospitalarios
                            </p>
                        </div>
                    </div>
                        ...
                        <DialogContent className="max-w-md">
                            <form onSubmit={handleCreate}>
                                <DialogHeader>
                                    ...
                                    <DialogDescription>
                                        Agregue un nuevo tipo de servicio hospitalario
                                    </DialogDescription>
                                </DialogHeader>
                                        ...
                                        <TableBody>
                                            {servicios.data.map((servicio) => (
                                                ...
                                                >
                                                    ...
                                                    <TableCell>
                                                        <span className="text-sm text-slate-700">{servicio.nombre}</span>
                                                    </TableCell>
                                                    <TableCell className="text-center">
                                                        <button
                                                            onClick={() => toggleStatus(servicio.id)}
                                    ...
                                    <div className="flex items-center justify-between pt-2">
                                        ...
                                        <div className="flex gap-2">
                                            <Button
                                                ...
                                                onClick={() => router.get(`/admin/configuracion/servicios?page=${servicios.current_page - 1}&search=${search}`)}
                                            >
                                                Anterior
                                            </Button>
                                            <Button
